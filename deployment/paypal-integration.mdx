...
5. **Pre-populates answer rows** for the new respondent: 14 blank `DemographicAnswer` rows (one per demographic question) and one `LikertAnswer` row per questionnaire item, with a default value of `6` (N/A), ready to be overwritten when the respondent takes the quiz.
