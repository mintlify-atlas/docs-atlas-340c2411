---
title: "Generating and Managing License Keys"
...
- Provides a **DOWNLOAD** button that POSTs to `downloadKeys.php`.
...
## Automatic Key Generation via PayPal IPN

In addition to manual bulk generation, the application also generates a single key automatically when a verified PayPal Instant Payment Notification (IPN) is received. The `authenticate_ipn.php` script calls the same `keygen()` function and inserts the resulting hashed key into the `License` table, then redirects the purchaser to `/purchaseSuccessful` where they can see their plain-text key. This path is fully automated and requires no admin interaction.
